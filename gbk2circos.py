...
def print_circos_record_file(record_list):
  i = 1
  f = open("circos_contigs.txt", "w")
  for record in record_list:
    if i%2 == 0:
      f.write("chr - %s %s 0 %s spectral-5-div-%s\n" % (record.contig, record.contig, len(record.seq), 3))
    else:
      #print "chr -", record.contig, record.contig, "0",len(record.seq), "spectral-5-div-%s" % (4)
      f.write("chr - %s %s 0 %s spectral-5-div-%s\n" % (record.contig, record.contig, len(record.seq), 4))
    i+=1
  f.close()

def print_circos_gene_file(record_list, type="CDS", strand ="1"):
  if strand == "1":
    f = open("circos_genes_plus.txt", "w")
  if strand == "-1":
    f = open("circos_genes_minus.txt", "w")
...
def print_circos_GC_file(record_list, type="CDS"):
  f = open("circos_genes_plus.txt", "w")
  for record in record_list:
    for feature in record.features:                                        
      if feature.type == type:   
        f.write("%s %s %s %s\n" % feature.contig, feature.start, feature.stop, feature.GC)
